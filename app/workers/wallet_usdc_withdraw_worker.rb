...
class WalletUsdcWithdrawWorker
  include Sidekiq::Job
  ...
  # Sends USDC from treasury to an external wallet address.
  # amount is in USDC (decimal), e.g. 5.0
  # Gas cap: 500 gwei
  MAX_GAS_PRICE = 500_000_000_000

  def perform(user_id, amount, to_address, _asset = "usdc")
    ...
    TreasuryNonceLock.with_nonce(rpc_url, sender) do |nonce|
      ...
      Rails.logger.info "WalletUsdcWithdraw: signing USDC tx [user=#{user_id}]"
    ...
    entry = wallet.wallet_ledger_entries.withdrawals.usdc_entries.order(created_at: :desc).first
    entry&.update(moncash_transaction_id: tx_hash)

    Rails.logger.info "WalletUsdcWithdraw: broadcast ok [user=#{user_id}]"
    notify_withdrawal_sent(user, amount, to_address, tx_hash)

  rescue => e
    Rails.logger.error "WalletUsdcWithdraw error [user=#{user_id}]: #{e.message}"
      ...
      begin
        ...
        if wallet
          WalletService.new(wallet).refund!(
            amount: amount.to_d,
            asset: "usdc",
            reason: "Retrè USDC echwe — ranbouse #{amount} USDC"
          )
          Rails.logger.info "WalletUsdcWithdraw: refunded #{amount} USDC [user=#{user_id}]"
          notify_withdrawal_failed(user, amount, e.message)
        end
      rescue => refund_error
        Rails.logger.error "WalletUsdcWithdraw refund failed [user=#{user_id}]: #{refund_error.message}"
      end
    else
      # Tx was broadcast but we got an error after — DO NOT refund, flag for manual review
      Rails.logger.error "WalletUsdcWithdraw: tx broadcast attempted (#{tx_hash || 'unknown'}), NOT refunding — needs manual review [user=#{user_id}]"
    end
  end

  def notify_withdrawal_sent(user, amount, to_address, tx_hash)
    user = User.find(user) if user.is_a?(Integer)
    WalletMailer.with(user_id: user.id, amount: amount.to_f, asset: "usdc", tx_hash: tx_hash, to_address: to_address)
               .withdrawal_sent.deliver_later
    NotificationService.crypto_withdrawal_sent(user, amount, "usdc", tx_hash)
  rescue => e
    Rails.logger.error "WalletUsdcWithdraw: withdrawal_sent notification failed [user=#{user.try(:id)}]: #{e.message}"
  end

  def notify_withdrawal_failed(user, amount, reason)
    user = User.find(user) if user.is_a?(Integer)
    clean_reason = humanize_rpc_error(reason)
    WalletMailer.with(user_id: user.id, amount: amount.to_f, asset: "usdc", reason: clean_reason)
               .withdrawal_failed.deliver_later
    NotificationService.crypto_withdrawal_failed(user, amount, "usdc", clean_reason)
  rescue => e
    Rails.logger.error "WalletUsdcWithdraw: withdrawal_failed notification failed [user=#{user.try(:id)}]: #{e.message}"
  end
  ...
  def sign_hash_with_retry(hash_bytes, key)
    ...
    MAX_SIGN_ATTEMPTS.times do |attempt|
      begin
        ...
        last_error = e
        Rails.logger.warn "WalletUsdcWithdraw: signing attempt #{attempt + 1} failed: #{e.message}, retrying..."
      end
    end
    raise last_error
  end
